...
export default function NewMissionaryPage() {
  ...
  return (
    <main className="p-8 max-w-3xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">선교사 등록</h1>
        <p className="text-sm text-gray-500 mt-1">새 선교사 정보를 입력합니다.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border p-8 space-y-6">
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input name="korean_name" value={form.korean_name} onChange={handleChange} required className={INPUT} placeholder="홍길동" />
            </div>
            <div>
              <label className={LABEL}>영문 이름</label>
              <input name="english_name" value={form.english_name} onChange={handleChange} className={INPUT} placeholder="Hong Gildong" />
            </div>
            <div>
              <label className={LABEL}>국가</label>
              <input name="country" value={form.country} onChange={handleChange} className={INPUT} placeholder="태국" />
            </div>
            <div>
              <label className={LABEL}>도시</label>
              <input name="city" value={form.city} onChange={handleChange} className={INPUT} placeholder="방콕" />
            </div>
            <div>
              <label className={LABEL}>사역 분야</label>
              <input name="ministry_type" value={form.ministry_type} onChange={handleChange} className={INPUT} placeholder="교회개척, 교육사역 등" />
            </div>
            <div>
        ...
        <section>
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea name="bio_ko" value={form.bio_ko} onChange={handleChange} rows={4} className={INPUT} placeholder="선교사 소개를 한국어로 입력하세요" />
            </div>
            <div>
              <label className={LABEL}>소개 (영문)</label>
              <textarea name="bio_en" value={form.bio_en} onChange={handleChange} rows={4} className={INPUT} placeholder="Enter missionary bio in English" />
            </div>
          </div>
        </section>
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={loading} className="bg-blue-950 text-white px-6 py-2.5 rounded-lg text-sm font-semibold disabled:opacity-50 hover:bg-blue-900 transition">
            {loading ? "저장 중..." : "저장하기"}
          </button>
