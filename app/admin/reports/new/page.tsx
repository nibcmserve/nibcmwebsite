...
export default function NewReportPage() {
  ...
  return (
    <main className="p-8 max-w-3xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">보고서 등록</h1>
        <p className="text-sm text-gray-500 mt-1">선교사 보고서를 작성합니다.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border p-8 space-y-5">
        ...
        <div>
          ...
          <input name="title" value={form.title} onChange={handleChange} required className={INPUT} placeholder="예: 2026년 5월 월간 보고서" />
        </div>

        <div>
          <label className={LABEL}>내용</label>
          <textarea name="content" value={form.content} onChange={handleChange} rows={10} className={INPUT} placeholder="이번 기간 사역 내용, 기도 제목, 특이 사항 등을 입력하세요" />
        </div>
